...
internal delegate nint WindowsWindowProcDelegate(
    nint hwnd,
    uint message,
    nint wParam,
    nint lParam,
...
internal sealed class WindowsHwndSource : CriticalFinalizerObject, IDisposable
{
    ...
    public static WindowsHwndSource FromHwnd(nint hwnd)
    {
        ...
        source.hWndProcHook = global::MS.Win32.NativeMethods.SetWindowLong(
            hwnd,
            GWLP_WNDPROC,
            dwNewLong
        );
    ...
    nint WndProcHook(nint hwnd, uint msg, nint wParam, nint lParam)
    {
        ...
        var result = global::MS.Win32.NativeMethods.CallWindowProc(
            hWndProcHook,
            hwnd,
            msg,
            wParam,
