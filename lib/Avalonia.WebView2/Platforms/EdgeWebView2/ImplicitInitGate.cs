...
sealed class ImplicitInitGate : ISupportInitialize
{
    ...
    {
        Trace.Assert(
            ISupportInitializeCount > 0,
            "Called EndInit more times than BeginInit was called."
        );
        --ISupportInitializeCount;
        OnDataChanged();
    }
    ...
    public void OnSynchronizationContextExists()
    {
        Trace.Assert(
            SyncContextOpen,
            "Expected UI thread to have a SynchronizationContext by the time this event fires."
        );
        OnDataChanged();
    }
