...
using Avalonia.PdfViewer.Internals;
using DynamicData;
using DynamicData.Binding;
using SkiaSharp;
using PdfConvert = PDFtoImage.Conversion;

namespace Avalonia.PdfViewer;
...
public partial class PdfViewer : UserControl, IDisposable
{
    public static readonly DirectProperty<PdfViewer, string?> SourceProperty =
        AvaloniaProperty.RegisterDirect<PdfViewer, string?>(
            nameof(Source),
            o => o.Source,
            (o, v) => o.Source = v
        );
    ...
    public static readonly DirectProperty<PdfViewer, bool> ShowSidebarProperty =
        AvaloniaProperty.RegisterDirect<PdfViewer, bool>(
            nameof(ShowSidebar),
            o => o.ShowSidebar,
            (o, v) => o.ShowSidebar = v
        );
    ...
    public static readonly DirectProperty<PdfViewer, Zoom?> ZoomProperty =
        AvaloniaProperty.RegisterDirect<PdfViewer, Zoom?>(
            nameof(Zoom),
            o => o.Zoom,
            (o, v) => o.Zoom = v
        );
    ...
    [
        Avalonia.PdfViewer.Zoom.Automatic,
        0.25,
        0.5,
        0.75,
        1,
    ...
    private readonly SourceCache<DrawableThumbnailImage, int> _thumbnailImagesCache = new(i =>
        i.PageNumber
    );
    ...
    private ReadOnlyObservableCollection<DrawableThumbnailImage> ThumbnailImages =>
        _thumbnailImages;
...
#pragma warning restore CA1416
    ...
    {
        CleanUpMainImage();
        if (_fileStream == null || index < 0)
            return;
#pragma warning disable CA1416
        MainImage.Source = PdfConvert
            .ToImage(_fileStream, new Index(index), leaveOpen: true)
            .ToAvaloniaImage();
#pragma warning restore CA1416
        ApplyZoom();
    }
    ...
    {
        if (ZoomCombobox.SelectedItem is not Zoom zoom)
            return;
        if (zoom > 0)
        {
            PercentageZoom(zoom);
    ...
    private void AutomaticZoom()
    {
        ...
        if (ZoomCombobox.SelectedItem is not Zoom currentZoom)
            return;
        if (width <= 0 || height <= 0)
            return;
    ...
    {
        if (ZoomCombobox.SelectedItem is not Zoom zoom || zoom != 0)
            return;
        AutomaticZoom();
    }
